...
def create_edge(source, target, relation, viz, time = None, font_size = None,time_precision='D'):
    ...
    match relation:
        case 'condition':
            if time:
                viz.edge(f'{source}:src',f'{target}:st', color='#FFA500', arrowhead='dotnormal', label=time, labelfontsize=font_size)
            else:
                viz.edge(f'{source}:src',f'{target}:st', color='#FFA500', arrowhead='dotnormal')
        case 'exclude':
            if source != target:
                viz.edge(f'{source}:src',f'{target}:st', color='#FC0C1B', arrowhead='normal', arrowtail='none', headlabel='%', labelfontcolor='#FC0C1B', labelfontsize='8')
            else:
                viz.edge(f'{source}:src:nw', f'{target}:st:ne', color='#FC0C1B', arrowhead='normal', arrowtail='none', headlabel='%', labelfontcolor='#FC0C1B', labelfontsize='8')
        case 'include':
            viz.edge(f'{source}:src',f'{target}:st', color='#30A627', arrowhead='normal', arrowtail='none', headlabel='+', labelfontcolor='#30A627', labelfontsize='10')
        case 'response':
            if time:
                viz.edge(f'{source}:src',f'{target}:st', color='#2993FC', arrowhead='normal', arrowtail='dot', dir='both', label=time, labelfontsize=font_size)
            else:
                viz.edge(f'{source}:src',f'{target}:st', color='#2993FC', arrowhead='normal', arrowtail='dot', dir='both')
        case 'noresponse':
            viz.edge(f'{source}:src',f'{target}:st', color='#7A514D', arrowhead='normal', headlabel='x', labelfontcolor='#7A514D', labelfontsize='8', arrowtail='dot', dir='both')
        case 'milestone':
            viz.edge(f'{source}:src',f'{target}:st', color='#A932D0', arrowhead='normal', headlabel='&#9671;', labelfontcolor='#A932D0', labelfontsize='8', arrowtail='dot', dir='both')
    return

def create_node(event,dcr,viz,font_size):
    try:
        roles = []
    ...
    # Define the HTML-like label using <TABLE> syntax
    label = f"""<
    <TABLE PORT="st" BORDER="1" STYLE="ROUNDED" CELLBORDER="0" CELLSPACING="0">
        <TR><TD>{roles}</TD><VR/><TD>{executed_record}{pending_record}</TD></TR>
        <HR/>
        <TR><TD COLSPAN="2" PORT="src">{label_map}</TD></TR>
    </TABLE>
    >"""
...
def apply(dcr: TimedDcrGraph, parameters):
    ...
    viz = Digraph("", filename=filename.name, engine='dot', graph_attr={'bgcolor': bgcolor, 'rankdir': set_rankdir, 'compound':'true'},
                  node_attr={'shape': 'none'}, edge_attr={'arrowsize': '0.5'})

    for event in dcr.events:
        if hasattr(dcr,'nestedgroups') and (event in dcr.nestedgroups_map or event in dcr.nestedgroups):
            pass
        else:
            create_node(event,dcr,viz,font_size)
    if hasattr(dcr, 'nestedgroups'):
        for nesting, events in dcr.nestedgroups.items():
            with viz.subgraph(name=f'cluster_{dcr.label_map[nesting]}') as g:
                g.attr(label=dcr.label_map[nesting])
                g.attr(style='filled', color='lightgrey')
                for nested_event in events:
                    create_node(nested_event,dcr,g,font_size)
