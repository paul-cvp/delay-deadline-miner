...
def export_dcr_graph(graph: DcrGraph, root, parents_dict=None, replace_whitespace_id='_', replace_whitespace_label=' ', time_precision='H', **kwargs):
    for event in graph.events:
        xml_event = etree.SubElement(root, "events")
        xml_event_id = etree.SubElement(xml_event, "id")
        xml_event_id.text = event.replace(' ', replace_whitespace_id)
        xml_event_label = etree.SubElement(xml_event, "label")
        xml_event_label.text = event.replace(' ', replace_whitespace_label)

        if parents_dict and event in parents_dict:
            xml_event_parent = etree.SubElement(xml_event, "parent")
            xml_event_parent.text = parents_dict[event].replace(' ', replace_whitespace_id)
        ...
        for event_prime in graph.events:
            if event in graph.conditions and event_prime in graph.conditions[event]:
                ...
                xml_source.text = event_prime.replace(' ', replace_whitespace_id)
                xml_target = etree.SubElement(xml_condition, "target")
                xml_target.text = event.replace(' ', replace_whitespace_id)
            ...
            if event in graph.responses and event_prime in graph.responses[event]:
                ...
                xml_source.text = event.replace(' ', replace_whitespace_id)
                xml_target = etree.SubElement(xml_response, "target")
                xml_target.text = event_prime.replace(' ', replace_whitespace_id)
                if hasattr(graph, 'timedresponses') and event in graph.timedresponses and event_prime in graph.timedresponses[event]:
                    ...
                    if time.floor(freq='S').to_numpy() > 0:
                        xml_duration = etree.SubElement(xml_response, "duration")
                        iso_time = time.floor(freq='S').isoformat()
                        if time_precision:
                            iso_time = iso_time.split(time_precision)[0] + time_precision
                        xml_duration.text = iso_time
            ...
            if hasattr(graph, 'milestones') and event in graph.milestones and event_prime in graph.milestones[event]:
                xml_milestone = etree.SubElement(root, "rules")
                xml_type = etree.SubElement(xml_milestone, "type")
                xml_type.text = "milestone"
                xml_source = etree.SubElement(xml_milestone, "source")
                xml_source.text = event.replace(' ', replace_whitespace_id)
                xml_target = etree.SubElement(xml_milestone, "target")
                xml_target.text = event_prime.replace(' ', replace_whitespace_id)
            if hasattr(graph, 'noresponses') and event in graph.noresponses and event_prime in graph.noresponses[event]:
                xml_coresponse = etree.SubElement(root, "rules")
                xml_type = etree.SubElement(xml_coresponse, "type")
                xml_type.text = "coresponse"
                xml_source = etree.SubElement(xml_coresponse, "source")
                xml_source.text = event.replace(' ', replace_whitespace_id)
                xml_target = etree.SubElement(xml_coresponse, "target")
                xml_target.text = event_prime.replace(' ', replace_whitespace_id)


def export_dcr_xml(graph: DcrGraph, output_file_name, dcr_title='DCR from pm4py', dcr_description=None, replace_whitespace_id='_', replace_whitespace_label=' ', time_precision='H'):
    ...
    if hasattr(graph, 'subprocesses'):
        for sp_name, sp_events in graph.subprocesses.items():
            xml_event = etree.SubElement(root, "events")
            xml_event_id = etree.SubElement(xml_event, "id")
            xml_event_id.text = sp_name.replace(' ', replace_whitespace_id)
            xml_event_label = etree.SubElement(xml_event, "label")
            xml_event_label.text = sp_name.replace(' ', replace_whitespace_label)
    ...
    if hasattr(graph, 'nestedgroups'):
        for n_name, n_events in graph.nestedgroups.items():
            xml_event = etree.SubElement(root, "events")
            xml_event_id = etree.SubElement(xml_event, "id")
            xml_event_id.text = n_name.replace(' ', replace_whitespace_id)
            xml_event_label = etree.SubElement(xml_event, "label")
            xml_event_label.text = n_name.replace(' ', replace_whitespace_label)
            xml_event_type = etree.SubElement(xml_event, "type")
            xml_event_type.text = "subprocess"# "nesting"
    ...
    export_dcr_graph(
        graph,
        root,
        parents_dict if parents_dict else None,
        replace_whitespace_id=replace_whitespace_id,
        replace_whitespace_label=replace_whitespace_label,
...
#
#
# def export_dcr_graph(graph : DcrGraph, root, parents_dict=None, replace_whitespace=None, time_precision='H', **kwargs):
#     '''
#
#     Parameters
#     ----------
...
#
#
# def export_dcr_xml(graph: DcrGraph, output_file_name, dcr_title='DCR from pm4py', dcr_description=None, replace_whitespace=' ',time_precision='H'):
#     '''
...
#             xml_event = etree.SubElement(root, "events")
#             xml_event_id = etree.SubElement(xml_event, "id")
#             xml_event_id.text = sp_name.replace(' ', replace_whitespace)
#             xml_event_label = etree.SubElement(xml_event, "label")
#             xml_event_label.text = sp_name.replace(' ', replace_whitespace)
#             xml_event_type = etree.SubElement(xml_event, "type")
#             xml_event_type.text = "subprocess"
#             for sp_event in sp_events:
#                 parents_dict[sp_event] = sp_name
...
#             xml_event = etree.SubElement(root, "events")
#             xml_event_id = etree.SubElement(xml_event, "id")
#             xml_event_id.text = n_name.replace(' ', replace_whitespace)
#             xml_event_label = etree.SubElement(xml_event, "label")
#             xml_event_label.text = n_name.replace(' ', replace_whitespace)
#             xml_event_type = etree.SubElement(xml_event, "type")
#             xml_event_type.text = "nesting"
#             for n_event in n_events:
#                 parents_dict[n_event] = n_name
#     if len(parents_dict) > 0:
#         export_dcr_graph(graph, root, parents_dict, replace_whitespace=replace_whitespace, time_precision=time_precision)
#     else:
#         export_dcr_graph(graph, root, None, replace_whitespace=replace_whitespace, time_precision=time_precision)
#
#     tree = etree.ElementTree(root)
#     tree.write(output_file_name, pretty_print=True)