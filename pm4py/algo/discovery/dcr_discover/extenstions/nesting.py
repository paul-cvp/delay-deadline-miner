    ...
    def mine(self, graph, parameters: Optional[Dict[str, Any]]):
        ...
        match nest_variant.value:
            case NestVariants.CHOICE.value:
                res = self.apply_choice(graph)
                for n in res.nestedgroups.keys():
                    res.label_map[n] = n #TODO: VCO use LLMs to get the most intuitive label
                return res
...
class Nesting(object):
    ...
    def get_nested_dcr_graph(self, graph, existing_nestings=None):
        ...
        all_nestings = set(existing_nestings).union(self.nesting_ids)
        res_dcr['labels'] = res_dcr['labels'].union(all_nestings)
        for n in all_nestings:
            res_dcr['labelMapping'][n] = n
